...
solve3A_2015 = do
    ...
    --let pt = solveShift cipherText
    --putStrLn $ "solveShift = " ++ (show $ pt)
    --let pt = solveVig cipherText
    --putStrLn $ "solveVig = " ++ (show $ pt)
    let km = cribMap "" ""
...
solve4A_2015 = do
    ...
    let ct  = clean isAlpha $ concat $ lines inCipherText
    --let fs = sortBy (flip compare) $ L.map swap $ toList $ count2freq $ countChars ct
    --putStrLn $ "Frequencies are: " ++ (show fs)
    --let c = ShiftCipher 'N'
    --let pt = decipher c ct
    -- putStrLn $ "plainText = " ++ (show $ pt)
...
topTriGrams n ct = fmap fst $ take n $ reverse $ sortWith snd $ toList $ count2freq $ loseZeros $ countTrigrams 3 ct
...
solve7B_2015 = do
    ...
    let ct  = clean isAlpha $ concat $ lines inCipherText
    let (pt, code) = last $ solveAmsco False 6 ct
    --putStrLn $ "cipherText = " ++ ct
